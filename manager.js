...
function checkManagerAccess() {
  ...
  if (!isManager) {
    ...
    if (
      currentUser &&
      currentUser.email === "manager@BusyCorner.com" &&
...
function initializeStats() {
  ...
  if (!stats || typeof stats !== "object") {
    ...
    if (!stats.yearly)
      stats.yearly = { year: currentYear, orders: 0, revenue: 0 };
  }
...
function checkAndResetStats() {
  ...
  if (
    stats.monthly.month !== currentMonth ||
    stats.monthly.year !== currentYear
  ) {
    stats.monthly = {
      month: currentMonth,
      year: currentYear,
    ...
    onlineOrders.forEach((order) => {
      ...
      if (!uniqueOrderIds.has(order.id)) {
        ...
        const formattedOrder = {
          ...
          pin:
            order.pin || Math.floor(100000 + Math.random() * 900000).toString(),
...
function updateDashboardStats() {
  ...
  const todayRevenueAmount = todayOrdersList.reduce(
  ...
  const weeklyRevenueAmount = weeklyOrdersList.reduce(
    (sum, order) => sum + (order.total || 0),
    0
  );
  ...
  const monthlyRevenueAmount = monthlyOrdersList.reduce(
    (sum, order) => sum + (order.total || 0),
    0
  );
  ...
  const yearlyRevenueAmount = yearlyOrdersList.reduce(
    (sum, order) => sum + (order.total || 0),
    0
  );

  // Get pending orders (including cash orders)
  const pendingOrdersList = currentOrders.filter(
  ...
  requestAnimationFrame(() => {
    ...
    if (weekRevenue)
      weekRevenue.textContent = formatCurrency(weeklyRevenueAmount);
    if (weekOrders) weekOrders.textContent = weeklyOrdersCount;
    if (monthRevenue)
      monthRevenue.textContent = formatCurrency(monthlyRevenueAmount);
...
function searchOrders(searchTerm) {
  const filteredOrders = orderManager.searchOrders(
    searchTerm,
    currentFilterStatus
  );
    ...
    orderElement.innerHTML = `
            <div class="order-item-header">
                ...
                <span class="order-date">${
                  order.timestamp ? formatDate(order.timestamp) : "Unknown date"
                }</span>
            </div>
      ...
      order.status || "pending"
            ...
            <div class="order-actions">
                <button class="status-btn view-order-btn" data-order-id="${
                  order.id
                }">View Details</button>
                ${
                  order.status === "pending"
                    ? `<button class="status-btn ready-btn" data-order-id="${order.id}" data-status="ready">Mark as Ready</button>`
                    : ""
                }
                ${
                  order.status === "ready"
                    ? `<button class="status-btn complete-btn" data-order-id="${order.id}" data-status="complete">Mark as Complete</button>`
                    : ""
                }
            </div>
        `;
...
function updateCashOrderDisplay() {
  ...
  // Calculate total
  const total = currentCashOrder.items.reduce(
...
function addItemToCashOrder() {
  ...
  const existingItemIndex = currentCashOrder.items.findIndex(
    (i) => i.id === item.id
  );
...
function showCashPaymentModal() {
  ...
  if (cashPaymentTotal)
    cashPaymentTotal.textContent = formatCurrency(currentCashOrder.total);
...
function processCashPayment() {
  ...
  showToast(
    `Cash order created successfully. Change: ${formatCurrency(
      cashOrder.change
    )}`
  );
}
  ...
  orderDetailsBody.innerHTML = `
        <div class="order-details-info">
            ...
            <div class="detail-row">
                ...
                <div class="detail-value">
                    <span class="order-status-display ${statusClass}">${
    order.status || "pending"
  }</span>
                </div>
            </div>
    ...
    itemElement.innerHTML = `
            ...
            <div class="item-footer">
                ...
                <div class="item-actions">
                    <button class="item-btn edit-item-btn" data-item-id="${
                      item.id
                    }">Edit</button>
                    <button class="item-btn delete-item-btn" data-item-id="${
                      item.id
                    }">Delete</button>
                </div>
            </div>
...
function showAddItemModal() {
  ...
  if (document.getElementById("itemAvailable"))
...
function saveManagerData(formData) {
  const newEmail = document.getElementById("newManagerEmail")
    ? document.getElementById("newManagerEmail").value.trim()
    : "";
  const currentPassword = document.getElementById("currentManagerPassword")
    ? document.getElementById("currentManagerPassword").value
    : "";
  const newPassword = document.getElementById("newManagerPassword")
    ? document.getElementById("newManagerPassword").value
    : "";
  const confirmPassword = document.getElementById("confirmManagerPassword")
...
// Update the downloadStatement function in manager.js to make statements read-only

// Download statement as Excel file - FIXED sheet name length and made read-only
function downloadStatement(statementTypeValue, includeProfitCalc) {
  ...
  const summaryData = [
    ...
    statementTypeValue === "weekly"
      ? [
          `Period: ${formatDateForStatement(
            filteredOrders[filteredOrders.length - 1]?.timestamp
          )} to ${formatDateForStatement(filteredOrders[0]?.timestamp)}`,
        ]
  ...
  if (includeProfitCalc) {
    const totalProfit = filteredOrders.reduce(
      (sum, order) => sum + calculateProfit(order.total || 0),
      0
    );
  ...
  // Protect the sheet (though this is a basic protection, not full security)
  if (!worksheet["!protect"]) {
    worksheet["!protect"] = {
      autoFilter: { 1: false },
      deleteColumns: { 1: false },
      deleteRows: { 1: false },
      formatCells: { 1: false },
      formatColumns: { 1: false },
      formatRows: { 1: false },
      insertColumns: { 1: false },
      insertRows: { 1: false },
      objects: { 1: false },
      pivotTables: { 1: false },
      scenarios: { 1: false },
      selectLockedCells: { 1: true },
      selectUnlockedCells: { 1: true },
      sort: { 1: false },
    };
  }
  ...
  // Add metadata to indicate read-only
  if (!workbook.Props) workbook.Props = {};
  workbook.Props.ReadOnlyRecommended = true;
  ...
  showToast(
    `${typeText} statement downloaded${
      includeProfitCalc ? " with profit calculation" : ""
    } (Read-Only)`
  );
}
...
if (itemForm)
  itemForm.addEventListener("submit", (e) => {
    ...
    const formData = {
      ...
      category: category,
      available: document.getElementById("itemAvailable")
...
if (managerForm)
  managerForm.addEventListener("submit", (e) => {
    ...
    const formData = {
      email: document.getElementById("newManagerEmail")
        ? document.getElementById("newManagerEmail").value.trim()
        : managerData.email,
    };
    ...
    if (saveManagerData(formData)) {
      ...
      // Clear form fields
      if (document.getElementById("newManagerEmail"))
        document.getElementById("newManagerEmail").value = "";
      if (document.getElementById("currentManagerPassword"))
        document.getElementById("currentManagerPassword").value = "";
      if (document.getElementById("newManagerPassword"))
        document.getElementById("newManagerPassword").value = "";
      if (document.getElementById("confirmManagerPassword"))
        document.getElementById("confirmManagerPassword").value = "";
...
if (addToCashOrderBtn)
  addToCashOrderBtn.addEventListener("click", addItemToCashOrder);

if (processCashPaymentBtn)
  processCashPaymentBtn.addEventListener("click", showCashPaymentModal);

// Cash payment modal functionality
if (closeCashPaymentModal)
...
if (confirmCashPayment)
  confirmCashPayment.addEventListener("click", processCashPayment);

// Cash amount received input
if (cashAmountReceived)
  cashAmountReceived.addEventListener("input", calculateCashChange);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  script.onerror = () => {
    ...
    showToast(
      "Failed to load Excel library. Please check your connection.",
      "error"
    );
    initialize();
  };
